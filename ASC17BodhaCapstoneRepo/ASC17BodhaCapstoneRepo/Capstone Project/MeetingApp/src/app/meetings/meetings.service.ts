...
export class MeetingsService {
  ...
  getMeetingsByDate(date:string){
    return this.http.get<IMeeting[]>(`${this.apiUrl}/timeframe/${date}`);
  }
  getMeetingsByAttendees(attendees:string){
    return this.http.get<IMeeting[]>(`${this.apiUrl}?attendees=${attendees}`);
  }
  getMeetingsByattendee(attendee:string){
    return this.http.get<IMeeting[]>(`${this.apiUrl}/ByAttendee/${attendee}`);
  }
